...
const (
	clientTemplateName = "client"
	clientFileName     = clientTemplateName + ".go"
)

var (
	clientTemplateText = `package {{.PackageName}}
...
`
	clientTemplate = template.Must(template.New(clientTemplateName).Funcs(templateHelpers).Parse(clientTemplateText))
)

func renderClient(in io.WriteCloser, data *apiDefinition) error {
	defer in.Close()
